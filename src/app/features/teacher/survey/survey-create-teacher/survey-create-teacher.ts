...
@Component({
  selector: 'app-survey-create-teacher',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './survey-create-teacher.html',
  styleUrl: './survey-create-teacher.css'
})
export class SurveyCreateTeacher {
  ...
  save(): void {
    ...
    this.surveyService.create(this.survey).subscribe({
      next: created => {
        ...
        this.router.navigate(['/app/teacher/surveys', created.id]);
      },
      error: err => {
        console.error(err);
        this.isSaving = false;
